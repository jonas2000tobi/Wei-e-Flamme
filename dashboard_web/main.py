...
app = FastAPI(title="Ebo Dashboard", version="0.1.0")
...
def _render_dashboard(data: dict[str, Any]) -> str:
    ...
    cards = "".join([
        _card("Profile", profiles.get("count", 0), "Mitgliederprofile"),
        _card("Events", events.get("count", 0), "im Snapshot"),
        _card("EC-Konten", (ec.get("balances") or {}).get("count", 0), "Balances"),
        _card("Auktionen", (loot.get("auctions") or {}).get("count", 0), "Loot/Auktionssystem"),
        _card("Need-User", (loot.get("needs") or {}).get("user_count", 0), "mit Needliste"),
        _card("Voice", voice.get("sessions_total", 0), f"offen: {voice.get('sessions_open', 0)}"),
        _card("Audit", audit.get("logs_total", 0), "Einträge"),
        _card("EC-Checks", event_checks.get("count", 0), "Anwesenheitschecks"),
    ])

    profile_rows = []
    for p in profiles.get("items") or []:
        profile_rows.append([p.get("display_name"), p.get("ingame_name"), p.get("main_role"), p.get("gearscore")])

    event_rows = []
    for ev in events.get("items") or []:
        event_rows.append([ev.get("title"), _dt(ev.get("when_iso")), ev.get("participant_count"), "ja" if ev.get("voice_enabled") else "nein"])

    balance_rows = []
    for b in (ec.get("balances") or {}).get("top") or []:
        balance_rows.append([b.get("display_name"), b.get("balance")])

    auction_rows = []
    for a in (loot.get("auctions") or {}).get("items") or []:
        leader = "—"
        if a.get("top_bid_amount") is not None:
            leader = f"User {a.get('top_bid_user_id')} / {a.get('top_bid_amount')} EC"
    ...
    for v in voice.get("recent_sessions") or []:
        voice_rows.append([v.get("member_name") or v.get("user_id"), v.get("channel_name") or v.get("channel_id"), _dt(v.get("joined_at")), _dt(v.get("left_at")), v.get("duration_seconds")])

    audit_rows = []
    for a in audit.get("recent_logs") or []:
        audit_rows.append([_dt(a.get("created_at")), a.get("action"), a.get("summary"), a.get("actor_id")])
    ...
    <section class="panel"><h2>👥 Mitgliederprofile</h2>{_table(['Name','Ingame','Rolle','GS'], profile_rows)}</section>
    <section class="panel"><h2>📅 Events</h2>{_table(['Event','Zeit','Teilnehmer','Voice'], event_rows)}</section>
    <section class="panel"><h2>🪙 EC Top-Konten</h2>{_table(['Spieler','EC'], balance_rows)}</section>
    <section class="panel"><h2>🎁 Auktionen</h2>{_table(['Item','Status','Phase','Gebote','Führend','Ende'], auction_rows)}</section>
    <section class="panel"><h2>🎙️ Voice-Sessions</h2>{_table(['Spieler','Kanal','Rein','Raus','Sekunden'], voice_rows)}</section>
    <section class="panel"><h2>🧾 Audit-Log</h2>{_table(['Zeit','Aktion','Zusammenfassung','Actor'], audit_rows)}</section>
    """
...
<head>
  ...
  <style>
    :root {{ --bg:#0f1014; --panel:#181a22; --panel2:#20232d; --text:#f1eadb; --muted:#a8a193; --gold:#d6a84f; --line:#333746; }}
    * {{ box-sizing:border-box; }}
    ...
    h1,h2 {{ margin:0 0 8px; }} p {{ color:var(--muted); }}
    ...
    .table-wrap {{ overflow-x:auto; }} table {{ width:100%; border-collapse:collapse; font-size:14px; }} th,td {{ padding:10px 8px; border-bottom:1px solid var(--line); text-align:left; vertical-align:top; }} th {{ color:var(--gold); font-size:12px; text-transform:uppercase; letter-spacing:.05em; }}
...
<body><main>{auth_note}{body}</main></body>
</html>"""
