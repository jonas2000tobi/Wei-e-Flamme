...
def _clean_item_display_name(value: Any) -> str:
    """Kurzer Itemname für Dashboard-Anzeigen.

    Bot-/Postgres-Rohwerte enthalten bei Needs oft Slot- und Dict-Metadaten, z. B.
    "1: Waffe 1: Daigons Sturmklinge (Schwert & Schild)" oder sogar die
    String-Repräsentation eines Dicts. Für die Oberfläche soll nur der Itemname
    sichtbar sein.
    """
    if value is None:
        return ""
    if isinstance(value, dict):
        for key in ("item_name", "raw_item_name", "item", "name", "label", "title", "short_name", "value", "text"):
            text = _clean_item_display_name(value.get(key))
            if text:
                return text
        return ""
    ...
    m = re.search(r"['\"](?:item_name|raw_item_name|item|name|label)['\"]\s*:\s*['\"]([^'\"]+)['\"]", text)
    if m:
        text = m.group(1).strip()

    text = html.unescape(text)
    text = re.sub(r"\s*✅\s*Erhalten\s*", "", text, flags=re.I).strip()
    text = re.sub(r"\s+", " ", text).strip(" -–—:\t\n\r")

    slot_words = (
        "waffe", "weapon", "umhang", "cape", "brosche", "kette", "halskette", "gürtel", "guertel",
        "ring", "armreif", "armband", "band", "hose", "handschuhe", "schuhe", "stiefel", "helm",
        "hut", "rüstung", "ruestung", "brust", "mantel", "anhänger", "anhaenger", "slot"
    )
    slot_re = r"(?:" + "|".join(slot_words) + r")"
    changed = True
    while changed:
        before = text
        text = re.sub(r"^\s*\d+\s*:\s*", "", text).strip()
        text = re.sub(rf"^\s*{slot_re}(?:\s*\d+)?\s*:\s*", "", text, flags=re.I).strip()
        text = re.sub(r"^\s*[A-Za-zÄÖÜäöüß ]+\s+\d+\s*:\s*", "", text).strip()
        changed = (text != before)

    text = re.sub(r"\s*\((?:Schwert & Schild|Schwert und Schild|Langbogen|Dolche|Kugel|Zauberstab|Stab|Armbrust|Großschwert|Grossschwert|Schild|Bogen|Dagger|Sword|Shield|Staff|Wand|Bow|Crossbow)[^)]*?\)\s*$", "", text, flags=re.I).strip()
    text = re.sub(r"\s*\((?:[^()]*)\)\s*$", "", text).strip()
    return text.strip(" -–—:")


def _phase3_need_label(slot_name: Any, item_name: Any) -> str:
    return _clean_item_display_name(item_name)
...
def _phase3_loot_pg_payload(guild_id: Any, snap: dict[str, Any]) -> Optional[dict[str, Any]]:
    ...
    for row in need_rows:
        ...
        item = _clean_item_display_name(row.get("item_name") or raw.get("item_name") or raw.get("item") or raw.get("name") or "")
    ...
    for row in auction_rows:
        ...
        item = _clean_item_display_name(row.get("item_name") or raw.get("item_name") or raw.get("item") or raw.get("name") or aid)
        auction = {
            **raw,
            "auction_id": aid,
            "item_name": item,
...
def _table(headers: list[str], rows: list[list[Any]], *, searchable: bool = True, placeholder: str = "Tabelle durchsuchen…") -> str:
    ...
    for row in rows:
        ...
        for idx, cell in enumerate(row):
            ...
            cells.append(f'<td data-label="{_e(label)}">{_cell(cell)}</td>')
...
def _need_list_html(title: str, items: Any) -> str:
    ...
    lis = "".join(f"<li>{_e(_loot_text(x))}</li>" for x in arr[:80])
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }} html {{ scroll-behavior:smooth; }}
    ...
    @media(max-width:560px) {{
      main.content {{ padding:12px 10px 42px; }} .grid,.analytics-grid,.side-nav {{ grid-template-columns:1fr; }} .bar-row {{ grid-template-columns:90px 1fr 38px; }} .home-item {{ grid-template-columns:38px minmax(0,1fr); }} .home-item .pill {{ grid-column:2; justify-self:start; }} .split {{ grid-template-columns:1fr; }}
    ...
    @media(max-width:900px) {{
      ...
      .brand-mark img {{ width:100%; height:100%; object-fit:contain; }}
      .brand-mark.sidebar-brand-logo {{ width:74px; height:52px; flex:0 0 74px; padding:0; border:0; border-radius:0; background:transparent; }}
...
function refreshNeedWeaponPicker(source) {{
  ...
  if (!picker) return;
  const selectionOnly = wrap.classList.contains('need-selection-only');
  let visible = 0;
  ...
  if (first) first.textContent = wanted
    ? (visible ? `2. Item auswählen ... (${{visible}})` : (selectionOnly ? 'Keine bekannten Items für diese Waffenart' : 'Keine bekannten Items für diese Waffenart – Freitext nutzen'))
    : '2. Item auswählen ...';
  if (!selectionOnly || source.dataset.changed === '1') picker.selectedIndex = 0;
...
def _loot_text(value: Any) -> str:
    ...
    if isinstance(value, (str, int, float)):
        return _clean_item_display_name(value)
    if isinstance(value, dict):
        for key in (
            "item_name", "raw_item_name", "item", "name", "label", "title", "display_name", "short_name",
            "ingame_name", "value", "text", "query", "loot_name",
        ):
            v = value.get(key)
            text = _clean_item_display_name(v)
            if text:
                return text
        for v in value.values():
            ...
            if text:
                return text
    return _clean_item_display_name(str(value))
...
def _need_slot_select_only_html(
    snap: dict[str, Any],
    *,
    input_id: str,
    slot: str,
    ...
    def option_rows(rows: list[dict[str, Any]], *, weapon_type: str = "") -> str:
        ...
        for item in rows[:1600]:
            ...
            if not item_name or item_name.casefold() in seen:
                continue
            if weapon_type:
                detected = _weapon_type_from_text(item_name)
...
def _render_loot_center(data: dict[str, Any], request: Optional[Request] = None, msg: str = "") -> str:
    ...
    for a in center["next_actions"][:80]:
        ...
        action_rows.append([
            ...
            (_auction_leader_or_roll_text(a.get("raw") or {}, snap) if isinstance(a.get("raw"), dict) else a.get("leader")),
    ...
    {drop_panel}
    <section class="panel"><h2>🔥 Nächste Loot-Aktionen</h2><p class="muted">Das ist die Arbeitsliste: Übergaben, offene Auktionen, Sale/Freie-Auktion-Kandidaten und aktive Auktionen ohne Gebote.</p>{_table(['Item','Bereich','Status','Gebote/Würfe','Führung/Wurf','Gewinner','Timer','Nächster Schritt'], action_rows, placeholder='Aktionen durchsuchen…')}</section>
    <section class="panel"><h2>🟢 Aktive Auktionen</h2>{_table(['Item','Bereich','Status','Gebote/Würfe','Führung/Wurf','Main/Second Need','Regel','Timer'], active_rows, placeholder='Aktive Auktionen durchsuchen…')}</section>
    <section class="split"><div class="panel"><h2>🏁 Übergabe offen</h2>{_table(['Item','Gewinner','Führend','Ende','Nächster Schritt'], handover_rows, placeholder='Übergaben durchsuchen…')}</div><div class="panel"><h2>🟡 Ohne Gebot / Sale prüfen</h2>{_table(['Item','Bereich','Main/Second Need','Timer','Hinweis'], no_bid_rows, placeholder='Ohne Gebot durchsuchen…')}</div></section>
...
def _item_damage_label(item: dict[str, Any]) -> str:
    if item.get("damage_min") is not None or item.get("damage_max") is not None:
        return f"{item.get('damage_min') or '—'} - {item.get('damage_max') or '—'}"
    if item.get("defense") is not None:
        return f"DEF {item.get('defense')}"
    return "—"


def _item_stats_html(item: dict[str, Any], max_items: int = 8) -> str:
    stats = item.get("stats") or {}
    if not isinstance(stats, dict) or not stats:
        return '<span class="muted">—</span>'
    parts: list[str] = []
    for k, v in list(stats.items())[:max_items]:
        if v is True or v in (None, ""):
            continue
        parts.append(f'<span class="item-chip"><b>{_e(k)}</b> {_e(v)}</span>')
    return "".join(parts) if parts else '<span class="muted">—</span>'


def _item_traits_html(item: dict[str, Any], max_items: int = 5) -> str:
    traits = item.get("traits") or []
    if not isinstance(traits, list) or not traits:
        return ""
    out: list[str] = []
    for tr in traits[:max_items]:
        if isinstance(tr, dict):
            name = str(tr.get("name") or "").strip()
            vals = tr.get("values") or []
            val_txt = " | ".join(str(x) for x in vals[:4]) if isinstance(vals, list) else str(vals or "")
            if name:
                out.append(f'<span class="item-chip trait"><b>{_e(name)}</b> {_e(val_txt)}</span>')
    return "".join(out)
...
def _render_need_builder_dashboard(
    data: dict[str, Any],
    request: Request,
    msg: str = "",
    *,
    ...
    if target_user_id:
        ...
        display_name = str(display_name_override or _profile_name_map(snap).get(user_id) or f"User {user_id}")
    ...
    <style>
      ...
      .need-builder-side h2,.nb-section h3{margin:0 0 12px;color:#d8e2ec;font-family:Georgia,serif;letter-spacing:.03em}.nb-builds{display:grid;gap:10px}.nb-build{display:grid;grid-template-columns:42px 1fr auto;gap:10px;align-items:center;padding:10px;border:1px solid rgba(255,255,255,.08);border-radius:14px;text-decoration:none;color:inherit;background:rgba(0,0,0,.18)}.nb-build.active{border-color:rgba(212,164,74,.75);background:linear-gradient(90deg,rgba(121,75,18,.32),rgba(0,0,0,.2))}.nb-build-icon{display:grid;place-items:center;width:40px;height:40px;border-radius:12px;background:rgba(120,88,180,.14);border:1px solid rgba(182,137,255,.3)}.nb-main-badge{display:block;color:#e7edf4;font-size:12px;margin-top:2px}.nb-build em{color:#a8b3bf;font-style:normal}.nb-side-actions{display:grid;gap:8px;margin-top:14px}.nb-side-actions .btn{width:100%;justify-content:center}.nb-toolbar{display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-bottom:16px}.nb-toolbar input{min-width:220px;padding:11px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.16);background:rgba(0,0,0,.25);color:inherit}.nb-toolbar .btn{min-height:42px}.nb-section{margin-top:18px;padding-top:14px;border-top:1px solid rgba(212,164,74,.16)}.nb-slot-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.nb-slot label{display:block;color:#c7d2de;font-weight:800;font-size:12px;margin:0 0 6px}.nb-picker{width:100%;display:grid;grid-template-columns:44px 1fr auto;gap:10px;align-items:center;text-align:left;border:1px solid rgba(255,255,255,.10);background:rgba(0,0,0,.28);color:inherit;border-radius:14px;padding:9px;min-height:62px;cursor:pointer}.nb-picker:hover,.nb-picker.nb-open{border-color:rgba(212,164,74,.72);box-shadow:0 0 0 2px rgba(212,164,74,.08)}.nb-thumb{display:grid;place-items:center;width:42px;height:42px;border-radius:10px;background:linear-gradient(135deg,rgba(82,45,122,.38),rgba(0,0,0,.2));border:1px solid rgba(182,137,255,.35);overflow:hidden}.nb-thumb img{width:100%;height:100%;object-fit:contain;padding:3px}.nb-picked{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#d9b4ff;font-weight:700}.nb-picker b{color:#c3d0dd}.nb-status{display:flex;gap:14px;flex-wrap:wrap;align-items:center;border:1px solid rgba(117,212,122,.22);background:rgba(27,91,35,.10);border-radius:14px;padding:12px;margin-bottom:16px}.nb-status b{color:#75d47a}.need-builder-memberpick{margin-bottom:14px}.need-builder-memberpick span{display:block;color:#a8b3bf;font-size:12px}.need-builder-memberpick select{width:100%;padding:10px;border-radius:12px;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.32);color:inherit}.nb-build-layout{display:grid;grid-template-columns:minmax(0,1fr) 310px;gap:14px;align-items:start}.nb-stats-panel{position:sticky;top:12px;border:1px solid rgba(212,164,74,.20);border-radius:16px;background:linear-gradient(135deg,rgba(12,16,23,.94),rgba(0,0,0,.66));padding:14px;box-shadow:0 16px 50px rgba(0,0,0,.24)}.nb-stats-panel h3{margin:0 0 10px;color:#d8e2ec;font-family:Georgia,serif}.nb-power{display:flex;align-items:center;justify-content:space-between;gap:10px;border:1px solid rgba(212,164,74,.18);border-radius:14px;padding:10px;background:rgba(212,164,74,.06);margin-bottom:12px}.nb-power strong{font-size:28px;color:#edf3f8}.nb-attrs{display:grid;gap:8px;margin-bottom:12px}.nb-attr{display:grid;grid-template-columns:1fr auto;gap:8px;border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:8px;background:rgba(255,255,255,.03)}.nb-attr b{font-size:16px}.nb-attr em{font-style:normal;color:#75d47a;font-size:12px}.nb-stat-list{display:grid;gap:6px;max-height:270px;overflow:auto}.nb-stat-row{display:grid;grid-template-columns:1fr auto;gap:8px;border-bottom:1px solid rgba(255,255,255,.06);padding:6px 2px}.nb-stat-row b{color:#d8e2ec}.nb-trait-list{display:grid;gap:6px;max-height:260px;overflow:auto}.nb-trait-line{border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:7px;background:rgba(255,255,255,.025)}.nb-trait-line b{display:block;color:#d8e2ec}.nb-trait-line span{color:#c7d2de;font-size:12px}.nb-calc-tabs{display:flex;gap:6px;margin:10px 0}.nb-calc-tabs button{flex:1;border:1px solid rgba(212,164,74,.22);border-radius:10px;background:rgba(0,0,0,.28);color:inherit;padding:8px;cursor:pointer}.nb-calc-tabs button.active{background:rgba(212,164,74,.18);color:#edf3f8;border-color:rgba(212,164,74,.65)}.nb-help{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:18px}.nb-help .soft{border:1px solid rgba(212,164,74,.16);border-radius:14px;padding:12px;background:rgba(0,0,0,.18)}.nb-picker-pop{position:fixed;z-index:9999;width:min(430px,calc(100vw - 24px));max-height:520px;border:1px solid rgba(212,164,74,.65);background:#0d1118;border-radius:16px;box-shadow:0 24px 90px rgba(0,0,0,.72);padding:10px}.nb-picker-pop input{width:100%;box-sizing:border-box;padding:12px;border-radius:12px;border:1px solid rgba(212,164,74,.35);background:rgba(0,0,0,.42);color:inherit;margin-bottom:8px}.nb-picker-list{max-height:400px;overflow:auto;display:grid;gap:6px}.nb-item-option{display:grid;grid-template-columns:38px 1fr auto;gap:9px;align-items:center;border:1px solid rgba(255,255,255,.07);background:rgba(255,255,255,.03);border-radius:12px;padding:7px;cursor:pointer}.nb-item-option:hover{border-color:rgba(212,164,74,.45);background:rgba(212,164,74,.08)}.nb-item-option img{width:36px;height:36px;object-fit:contain;border-radius:8px;background:rgba(0,0,0,.3);border:1px solid rgba(182,137,255,.26)}.nb-item-option strong{display:block;color:#e1c0ff;font-size:13px}.nb-item-option small{color:#a8b3bf}.nb-item-option em{font-style:normal;color:#c3d0dd}.nb-empty{padding:14px;color:#a8b3bf}.nb-mobile-note{display:none;color:#a8b3bf}.hero .nb-top-actions{display:flex;gap:8px;flex-wrap:wrap}.btn.gold{background:linear-gradient(180deg,#d8e1ea,#6f8296);color:#111821;border-color:#c3d0dd;font-weight:900}.nb-values-search{width:100%;box-sizing:border-box;margin:8px 0 10px;padding:9px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.12);background:rgba(0,0,0,.32);color:inherit}.nb-stat-groups{display:grid;gap:8px;max-height:520px;overflow:auto;padding-right:2px}.nb-stat-group{border:1px solid rgba(255,255,255,.07);border-radius:12px;background:rgba(255,255,255,.025);overflow:hidden}.nb-stat-group h4{margin:0;padding:8px 10px;color:#d8e2ec;background:rgba(212,164,74,.055);font-size:13px}.nb-value-row{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:baseline;padding:6px 10px;border-top:1px solid rgba(255,255,255,.055);font-size:13px}.nb-value-row b{color:#e0e7ee}.nb-value-row small{color:#8edb82;margin-left:6px}.nb-breakpoints{display:grid;gap:4px;margin:8px 0 10px}.nb-bp{display:grid;grid-template-columns:1fr auto;gap:8px;border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:7px;background:rgba(0,0,0,.18);font-size:12px}.nb-bp b{color:#d8e2ec}.nb-bp span{color:#a8b3bf;text-align:right}.nb-item-tooltip{position:fixed;z-index:10050;width:min(360px,calc(100vw - 26px));pointer-events:none;border:1px solid rgba(212,164,74,.72);border-radius:15px;background:linear-gradient(145deg,#111722,#080b10);box-shadow:0 22px 80px rgba(0,0,0,.78);padding:12px;color:#f3ead8}.nb-tip-head{display:grid;grid-template-columns:54px 1fr;gap:10px;align-items:center;margin-bottom:8px}.nb-tip-head img{width:52px;height:52px;object-fit:contain;border-radius:10px;background:rgba(0,0,0,.35);border:1px solid rgba(182,137,255,.32)}.nb-tip-head strong{display:block;color:#e1c0ff}.nb-tip-head small{color:#a8b3bf}.nb-tip-sec{border-top:1px solid rgba(255,255,255,.08);padding-top:8px;margin-top:8px}.nb-tip-sec h5{margin:0 0 5px;color:#d8e2ec;font-size:12px;text-transform:uppercase;letter-spacing:.04em}.nb-tip-row{display:grid;grid-template-columns:1fr auto;gap:8px;font-size:12px;padding:3px 0}.nb-tip-row b{color:#e0e7ee}.nb-tip-trait{font-size:12px;border:1px solid rgba(255,255,255,.06);border-radius:8px;padding:5px;margin-top:4px;background:rgba(255,255,255,.025)}.nb-tip-trait b{display:block;color:#d8e2ec}.nb-item-option{position:relative}.nb-item-option:hover strong{color:#edf3f8}.nb-stats-note{font-size:12px;color:#a8b3bf;margin:8px 0 0}.nb-panel-subtabs{display:flex;gap:6px;margin:10px 0}.nb-panel-subtabs button{flex:1;border:1px solid rgba(212,164,74,.22);border-radius:10px;background:rgba(0,0,0,.28);color:inherit;padding:8px;cursor:pointer}.nb-panel-subtabs button.active{background:rgba(212,164,74,.18);color:#edf3f8;border-color:rgba(212,164,74,.65)}

      /* Needliste kompakt-breit: Questlog-Feeling, aber ohne horizontales Wegdrücken */
      main.content{max-width:100%!important;width:100%!important;box-sizing:border-box;padding-left:14px!important;padding-right:14px!important;overflow-x:hidden}
      .need-builder-shell{grid-template-columns:230px minmax(0,1fr);gap:12px;width:100%;max-width:100%;overflow:hidden}
      .need-builder-side,.need-builder-main{border-radius:18px;min-width:0}
      .need-builder-side{padding:12px}
      .need-builder-main{padding:14px 16px;overflow:hidden}
      .need-builder-side h2{font-size:20px;margin-bottom:10px}
      .nb-builds{gap:8px}.nb-build{grid-template-columns:34px 1fr auto;gap:8px;padding:8px;border-radius:12px}.nb-build-icon{width:32px;height:32px;border-radius:10px}.nb-build strong{font-size:14px}.nb-main-badge{font-size:11px}
      .nb-side-actions{margin-top:9px;gap:7px}.nb-side-actions .btn{width:100%;min-height:34px;padding:7px 9px;font-size:13px;line-height:1.15;border-radius:10px}.nb-side-actions .btn small{display:none}.need-builder-side p.muted{font-size:12px;line-height:1.35;margin-top:10px}
      .need-builder-memberpick{margin-bottom:10px}.need-builder-memberpick select{padding:8px 9px;border-radius:10px;font-size:13px}
      .nb-toolbar{gap:7px;margin-bottom:12px}.nb-toolbar input{min-width:160px;max-width:220px;padding:8px 10px;border-radius:10px}.nb-toolbar .btn{min-height:34px;padding:7px 10px;font-size:13px;border-radius:10px;white-space:nowrap}
      .nb-status{gap:10px;padding:9px 11px;margin-bottom:12px;border-radius:12px;font-size:13px}
      .nb-build-layout{grid-template-columns:minmax(0,1fr) minmax(280px,330px);gap:12px;width:100%;max-width:100%;overflow:hidden}
      .nb-section{margin-top:14px;padding-top:11px}.nb-section h3{font-size:21px;margin-bottom:10px}
      .nb-slot-grid{grid-template-columns:repeat(2,minmax(190px,1fr));gap:10px}.nb-slot label{font-size:11px;margin-bottom:5px}
      .nb-picker{min-height:54px;grid-template-columns:38px minmax(0,1fr) auto;gap:8px;padding:7px 8px;border-radius:12px}.nb-thumb{width:36px;height:36px;border-radius:9px}.nb-picked{font-size:13px;min-width:0}.nb-picker b{font-size:12px}
      .nb-stats-panel{padding:12px;max-height:calc(100vh - 92px);overflow:hidden;border-radius:14px}.nb-stats-panel h3{font-size:18px}.nb-power{padding:8px;border-radius:11px;margin-bottom:8px}.nb-power strong{font-size:22px}.nb-values-search{padding:8px 9px}.nb-panel-subtabs button{padding:7px 6px;font-size:12px}.nb-stat-groups{max-height:500px}.nb-value-row{font-size:12px;padding:5px 8px}.nb-stat-group h4{font-size:12px;padding:7px 8px}
      .nb-picker-pop{width:min(500px,calc(100vw - 24px));max-height:min(640px,calc(100vh - 28px))}.nb-picker-list{max-height:500px}.nb-item-option{grid-template-columns:42px 1fr auto;padding:8px}.nb-item-option img{width:40px;height:40px}.nb-item-option strong{font-size:13px}
      .nb-item-tooltip{width:min(430px,calc(100vw - 26px))}.nb-tip-head{grid-template-columns:58px 1fr}.nb-tip-head img{width:56px;height:56px}
      .nb-help{grid-template-columns:1fr 1fr;gap:10px;margin-top:14px}.nb-help .soft{padding:10px}
      @media(max-width:1680px){.nb-build-layout{grid-template-columns:1fr}.nb-stats-panel{position:relative;top:auto;max-height:none}.nb-stat-groups{max-height:420px}.nb-slot-grid{grid-template-columns:repeat(2,minmax(180px,1fr))}}
      @media(max-width:1280px){.need-builder-shell{grid-template-columns:210px minmax(0,1fr);gap:10px}.nb-toolbar input{max-width:180px}.nb-slot-grid{grid-template-columns:1fr 1fr}}
      @media(max-width:1120px){.need-builder-shell{grid-template-columns:1fr}.need-builder-side{display:grid;grid-template-columns:minmax(180px,240px) 1fr;gap:10px;align-items:start}.need-builder-side h2{grid-column:1/-1}.nb-builds{grid-column:1}.nb-side-actions{margin-top:0}.nb-build-layout{grid-template-columns:1fr}}
      @media(max-width:980px){.need-builder-shell{grid-template-columns:1fr}.nb-slot-grid{grid-template-columns:1fr 1fr}.nb-help{grid-template-columns:1fr}.nb-mobile-note{display:block}.need-builder-side{display:block}}
    ...
    builder_header = "" if embedded else """
    <nav class="topnav"><a href="/">← Übersicht</a><a href="/items">Item-Datenbank</a><a href="/loot">Loot</a><a href="/members">Mitglieder</a></nav>
    <section class="hero">
      <div><div class="eyebrow">Charakter-Builder</div><h1>🧰 Charakter-Builder</h1><p class="muted">Mehrere Builds pro Spieler. Nur der aktive Main-Need-Build wird an den Bot übertragen.</p></div>
    ...
    <div class="need-builder-shell">
      <aside class="need-builder-side">
        ...
        <form class="nb-side-actions" method="post" action="/needs/builds/new">
          <input type="hidden" name="user_id" value="{int(user_id)}"><button class="btn" type="submit">＋ Neuer Build</button>
        </form>
        <form class="nb-side-actions" method="post" action="/needs/builds/main">
          ...
          <button class="btn gold" type="submit">★ Main Need</button>
        </form>
      ...
      <main class="need-builder-main">
        ...
        <form method="post" action="/needs/builds/save" id="needBuilderForm">
          ...
          <div class="nb-toolbar">
            ...
            <button class="btn" type="submit" formaction="/needs/builds/duplicate">⧉ Duplizieren</button>
    ...
    <script>
      ...
      const NB_GROUPS = [
        ['Angriff', ['Min. Schaden','Max. Schaden','Reichweite','Angriffstempo','Zusatzschaden','Spezies-Schadensbonus','Chance auf Zweitwaffenangriff']],
        ...
        ['Ressourcen', ['Max. Gesundheit','Gesundheitsregeneration','Schildgesundheit','Max. Mana','Manaregeneration']],
      ...
      const NB_ALIASES = {{
        ...
        'Max Health':'Max. Gesundheit','Max. Mana':'Max. Mana','Mana Regen':'Manaregeneration','Health Regen':'Gesundheitsregeneration','Shield Health':'Schildgesundheit',
        ...
        'Attack Speed':'Angriffstempo','Range':'Reichweite','Bonus Damage':'Zusatzschaden','Species Damage Bonus':'Spezies-Schadensbonus','Off-Hand Weapon Attack Chance':'Chance auf Zweitwaffenangriff','Buff Duration':'Buff-Dauer','Skill Damage Bonus':'Fähigkeitsschaden-Bonus',
      ...
      function nbIsMobileLayout() {{ return window.innerWidth <= 760; }}
      function nbClosePicker() {{ if (nbPop) {{ nbPop.remove(); nbPop = null; }} document.body.classList.remove('nb-picker-open'); document.querySelectorAll('.nb-picker.nb-open').forEach(x=>x.classList.remove('nb-open')); }}
      function nbOpenPicker(btn) {{
        ...
        if (nbIsMobileLayout()) {{
          nbPop.classList.add('mobile');
          document.body.classList.add('nb-picker-open');
        }} else {{
          nbPop.style.left = Math.min(rect.left, window.innerWidth - 580) + 'px';
          nbPop.style.top = Math.min(rect.bottom + 8, window.innerHeight - 680) + 'px';
        }}
      ...
      function nbSlotTip(btn, ev) {{
        if (!nbCanHover() || nbIsMobileLayout()) return;
      ...
      function nbMoveTip(ev) {{
        if (!nbTip || !ev || nbIsMobileLayout()) return;
        const pad = 16;
...
def _need_builder_profile_redirect(
    request: Request,
    user_id: int,
    *,
    build_id: str = "",
...
async def need_build_new(request: Request, _: bool = Depends(_auth)):
    ...
    return RedirectResponse(url=_need_builder_profile_redirect(request, int(user_id), build_id=bid, msg="Build erstellt"), status_code=303)
...
async def need_build_save(request: Request, _: bool = Depends(_auth)):
    ...
    return RedirectResponse(url=_need_builder_profile_redirect(request, int(user_id), build_id=bid, msg="Build gespeichert"), status_code=303)
...
async def need_build_save_main(request: Request, _: bool = Depends(_auth)):
    ...
    return RedirectResponse(url=_need_builder_profile_redirect(request, int(user_id), build_id=bid, msg=f"Build gespeichert und {queued} Bot-Aufträge angelegt"), status_code=303)
...
async def need_build_main(request: Request, _: bool = Depends(_auth)):
    ...
    actor = _current_user(request) or {"user_id": "basic-admin", "username": "Basic Admin"}
    queued = _need_builder_queue_main(int(guild_id), int(user_id), bid, actor)
    return RedirectResponse(url=_need_builder_profile_redirect(request, int(user_id), build_id=bid, msg=f"Main Need gesetzt: {queued} Bot-Aufträge"), status_code=303)
...
async def need_build_duplicate(request: Request, _: bool = Depends(_auth)):
    ...
    return RedirectResponse(url=_need_builder_profile_redirect(request, int(user_id), build_id=new_id, msg="Build dupliziert"), status_code=303)
...
async def need_build_delete(request: Request, _: bool = Depends(_auth)):
    ...
    return RedirectResponse(url=_need_builder_profile_redirect(request, int(user_id), msg="Build gelöscht"), status_code=303)
...
def _guild_membership_duration(value: Any) -> str:
    joined = _dt_obj(value)
    if not joined:
        return "—"
    ...
    years = int(days // 365)
    remaining = int(days - years * 365)
    months = int(remaining // 30)
    rest_days = int(remaining - months * 30)
    parts: list[str] = []
    if years:
        parts.append(f"{years} Jahr" if years == 1 else f"{years} Jahren")
    if months and len(parts) < 2:
        parts.append(f"{months} Monat" if months == 1 else f"{months} Monaten")
    if not parts or (years == 0 and months == 0):
        parts.append(f"{rest_days} Tag" if rest_days == 1 else f"{rest_days} Tagen")
    return "seit " + ", ".join(parts[:2])
...
def _render_member_portal(data: dict[str, Any], user_id: int, request: Request, msg: str = "") -> str:
    ...
    for row in profiles:
        if isinstance(row, dict) and _user_id(row.get("user_id") or row.get("member_id") or row.get("discord_id")) == uid:
            profile = row
            break
    stored_profile = profile.get("profile") if isinstance(profile.get("profile"), dict) else {}

    def profile_value(*keys: str, default: Any = "") -> Any:
        for key in keys:
    ...
    display = str(
        profile_value("display_name", "server_name", "discord_name", "discord_username", "ingame_name")
        or names.get(uid)
        or f"User {uid}"
    )
    ...
    if not guild_rank:
        role_names = [str(r.get("role_name") or "") for r in (profile.get("roles") or []) if isinstance(r, dict)]
...
def _discord_feed_escape_content(text: Any) -> str:
...
def _render_discord_feed_page(data: dict[str, Any], *, key: str, title: str, subtitle: str, env_hint: str) -> str:
    snap = data.get("snapshot") or {}
...
def tnl_news_page(_: bool = Depends(_auth)) -> HTMLResponse:
...
def tnl_guides_page(_: bool = Depends(_auth)) -> HTMLResponse:
    return HTMLResponse(_render_discord_feed_page(
        _snapshot_payload(),
        key="guides",
        title="📚 TnL Guides",
