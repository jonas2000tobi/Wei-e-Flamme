...
def _render_need_builder_dashboard(data: dict[str, Any], request: Request, msg: str = "") -> str:
    ...
    <style>
      ...
      /* Questlog-ähnlicher breiter Build-Planner nur auf /needs */
      main.content{max-width:1840px!important;padding-left:18px!important;padding-right:18px!important}
      .need-builder-shell{grid-template-columns:300px minmax(0,1fr);gap:18px}
      .need-builder-side,.need-builder-main{border-radius:20px}
      .need-builder-main{padding:18px 20px}
      .nb-build-layout{grid-template-columns:minmax(880px,1fr) 390px;gap:18px}
      .nb-slot-grid{grid-template-columns:repeat(3,minmax(230px,1fr));gap:14px}
      .nb-picker{min-height:70px;grid-template-columns:52px 1fr auto;padding:10px 12px}
      .nb-thumb{width:50px;height:50px;border-radius:12px}.nb-picked{font-size:15px}
      .nb-stats-panel{padding:16px;max-height:calc(100vh - 90px);overflow:hidden}.nb-stat-groups{max-height:600px}
      .nb-picker-pop{width:min(560px,calc(100vw - 24px));max-height:min(660px,calc(100vh - 28px))}.nb-picker-list{max-height:520px}
      .nb-item-option{grid-template-columns:48px 1fr auto;padding:9px}.nb-item-option img{width:46px;height:46px}.nb-item-option strong{font-size:14px}
      .nb-item-tooltip{width:min(460px,calc(100vw - 26px))}.nb-tip-head{grid-template-columns:62px 1fr}.nb-tip-head img{width:60px;height:60px}
      @media(max-width:1420px){.nb-build-layout{grid-template-columns:1fr}.nb-stats-panel{position:relative;top:auto;max-height:none}.nb-stat-groups{max-height:520px}}
