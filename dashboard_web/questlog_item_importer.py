...
def default_weapon_urls(include_heroic: bool = True) -> list[str]:
    return _merge_unique_urls(DEFAULT_WEAPON_CATEGORY_URLS, [_change_grade_in_url(u, 51) for u in DEFAULT_WEAPON_CATEGORY_URLS] if include_heroic else [])

def default_armor_urls(include_heroic: bool = True) -> list[str]:
    return _merge_unique_urls(DEFAULT_ARMOR_CATEGORY_URLS, [_change_grade_in_url(u, 51) for u in DEFAULT_ARMOR_CATEGORY_URLS] if include_heroic else [])

def default_accessory_urls(include_heroic: bool = True) -> list[str]:
    return _merge_unique_urls(DEFAULT_ACCESSORY_CATEGORY_URLS, [_change_grade_in_url(u, 51) for u in DEFAULT_ACCESSORY_CATEGORY_URLS] if include_heroic else [])
...
RARITY_RANK = {"Common": 10, "Uncommon": 20, "Rare": 30, "Epic": 40, "Heroic": 45, "Legendary": 50}
...
def force_weapon_grade_filter(url: str) -> str:
    ...
    Ein expliziter Filter wie grade=51 für Heroisch bleibt erhalten.
    Historischer Funktionsname bleibt aus Kompatibilität erhalten.
    """
    if is_items_url(url) and classify_main_category(url) in {"weapon", "armor", "accessory"}:
        if has_grade_filter(url):
            return url
...
def expand_seed_url(raw_url: str) -> list[str]:
    ...
    if classify_main_category(url) == "weapon" and is_category_list_url(url):
        return [force_weapon_grade_filter(u.rstrip("/")) for u in default_weapon_urls()]
    if classify_main_category(url) == "armor" and is_category_list_url(url):
        return [force_weapon_grade_filter(u.rstrip("/")) for u in default_armor_urls()]
    if classify_main_category(url) == "accessory" and is_category_list_url(url):
        return [force_weapon_grade_filter(u.rstrip("/")) for u in default_accessory_urls()]
    return [url]
...
def run_import(args: argparse.Namespace) -> int:
    ...
    with sync_playwright() as pw:
        ...
        if preset in {"weapon", "weapons", "waffen", "waffe"}:
            seeds = [CategorySeed(force_weapon_grade_filter(u.rstrip("/")), "weapon") for u in default_weapon_urls()]
            ...
            seeds = [CategorySeed(force_weapon_grade_filter(u.rstrip("/")), "armor") for u in default_armor_urls()]
