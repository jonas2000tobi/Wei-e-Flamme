...
# ========= DM-RSVP-Modul (robust import, egal ob in "bot/" oder flach) =========
try:
...
async def on_ready():
    ...
    await tree.sync()
    # === DM-RSVP Kommandos registrieren ===
    try:
        await setup_rsvp_dm(client, tree)
    except Exception as e:
        print("setup_rsvp_dm failed:", e)
...
async def on_guild_join(guild: discord.Guild):
    try:
        ...
        # DM-RSVP Kommandos für neue Guild
        try:
            await setup_rsvp_dm(client, tree)
        except Exception as e:
            print("setup_rsvp_dm (guild_join) failed:", e)
    except Exception as e:
...
async def on_member_join(member: discord.Member):
    if member.bot: return
    # Onboarding-DM (dein bisheriges Verhalten)
    await _send_onboarding_dm(member)
    # NEU: Auto-Resend der Event-DMs für aktuelle/nicht abgelaufene Events
    try:
