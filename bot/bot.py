...
# ---- Minimale Topliste + Filter (keine Bots, optional Gildenrolle) ----
...
async def _monthly_reset_if_due(now: datetime):
    # Reset am letzten Tag des Monats um 00:00 (Europe/Berlin)
...
async def wf_admin_sync(inter: discord.Interaction):
    if not is_admin(inter):
        ...
        return
    try:
        # 1) Global sync (falls du mal global nutzt)
        await tree.sync()
        # 2) Globals -> Guild kopieren und sofort syncen (sofort sichtbar)
...
@tree.command(name="wf_admin_sync_hard", description="Harter Re-Sync (löscht & lädt neu, kopiert Globals).")
...
async def on_ready():
    ...
    # --- WICHTIG: Befehle sofort sichtbar machen ---
    try:
        await tree.sync()  # global (optional)
        for g in client.guilds:
