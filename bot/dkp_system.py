...
def _leadership_role_ids(guild_id: int) -> set[int]:
    ...
    if _runtime_db is not None:
        try:
            settings = _runtime_db.get_all_guild_settings(gid)
        except Exception:
            settings = {}

    legacy: dict[str, set[int]] = {"leader": set(), "advisor": set(), "guardian": set()}
    try:
        c = _load_leader_cfg().get(str(gid)) or {}
        ...
        pass
    try:
        c = _load_portal_cfg().get(str(gid)) or {}
...
def _gcfg(guild_id: int) -> dict:
    ...
    if _runtime_db is not None:
        try:
            settings = _runtime_db.get_all_guild_settings(int(guild_id))
            if "guild_channel_ec_log_id" in settings:
                c["log_channel_id"] = int(settings.get("guild_channel_ec_log_id", 0) or 0)
        except Exception:
            pass
...
def _reserve_points(guild_id: int, event_type: str) -> int:
    # Gildenregel: Reserve bekommt immer fix 5 EC, unabhängig vom Eventwert.
...
def _member_role_id(home_guild_id: int) -> int:
    if _runtime_db is not None:
        try:
            settings = _runtime_db.get_all_guild_settings(int(home_guild_id))
            if DASHBOARD_MEMBER_ROLE_SETTING in settings:
                return int(settings.get(DASHBOARD_MEMBER_ROLE_SETTING, 0) or 0)
        except Exception:
            pass
...
async def _process_dashboard_ec_award_request(client: discord.Client, row: dict) -> None:
    ...
    try:
        ...
        if int(guild_id) != int(home_id):
            _finish_dashboard_ec_award_request(request_id, "rejected", {"ok": False, "error": "EC wird nur auf dem Home-Gildenserver gebucht.", "guild_id": guild_id, "home_id": home_id})
            return
        ...
        for item in rows:
            ...
            if not _is_ebolus_member(client, int(home_id), uid):
                skipped.append({**item, "reason": "keine Gildenrolle / nicht im Home-Server"})
                continue
            tx = _add_transaction(
                int(home_id),
                uid,
...
async def dkp_event_check_loop():
    ...
    for event_id, obj in list(store.items()):
        ...
        home_id = _home_guild_id(default=guild_id)
        if int(guild_id) != int(home_id):
            # EC ist gildenintern. Allianz-Master liegt auf dem Home-Server.
            continue
...
async def setup_dkp_system(client: discord.Client, tree: app_commands.CommandTree):
    ...
    async def dkp_award_event(inter: discord.Interaction, event_id: str, confirm: bool = False, event_type: Optional[app_commands.Choice[str]] = None):
        ...
        home_id = _home_guild_id(default=inter.guild.id)
        if int(inter.guild.id) != int(home_id):
            await inter.followup.send("❌ EC/DKP wird nur auf dem Home-Gildenserver vergeben.", ephemeral=True)
            return
        event = rsvp.get_attendance_event(int(home_id), str(event_id))
        if not event:
    ...
    async def dkp_post_event_check(inter: discord.Interaction, event_id: str):
        ...
        home_id = _home_guild_id(default=inter.guild.id)
        if int(inter.guild.id) != int(home_id):
            await inter.followup.send("❌ EC/DKP wird nur auf dem Home-Gildenserver verwaltet.", ephemeral=True)
            return
...
def _award_log_embed(event: dict, event_type: str, present: list[dict], reserve: list[dict], skipped_partner: list[dict], skipped_open: list[dict], actor: discord.abc.User) -> discord.Embed:
    ...
    emb.add_field(name="Summe", value=f"**{total} EC** an **{len(present) + len(reserve)}** Gildenspieler", inline=False)
