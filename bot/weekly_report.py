...
def _guild_brand_name(guild: discord.Guild) -> str:
    try:
        if central_guild_config is not None:
            return central_guild_config.display_name(guild, guild.name)
    except Exception:
        pass
    return guild.name
...
def _gcfg(guild_id: int) -> dict:
    ...
    try:
        if central_guild_config is not None:
            if central_guild_config.channel_mapping_configured(int(guild_id), "weekly_report"):
                c["channel_id"] = central_guild_config.channel_id(int(guild_id), "weekly_report")
    except Exception:
        pass
    cfg[str(guild_id)] = c
    return c
...
async def _leader_contact_status_counts(guild: discord.Guild) -> dict:
    ...
    try:
        if central_guild_config is not None and central_guild_config.channel_mapping_configured(guild.id, "leader_contact_internal"):
            internal_channel_id = central_guild_config.channel_id(guild.id, "leader_contact_internal")
    except Exception:
        internal_channel_id = 0
    central_internal_configured = bool(
        central_guild_config is not None
        and central_guild_config.channel_mapping_configured(guild.id, "leader_contact_internal")
    )
    if not internal_channel_id and not central_internal_configured:
...
async def build_weekly_report_embed(guild: discord.Guild) -> discord.Embed:
    ...
    emb = discord.Embed(
        title=f"📋 Wochenbericht – {_guild_brand_name(guild)}",
        description=(
