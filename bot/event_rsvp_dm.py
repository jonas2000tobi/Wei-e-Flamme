...
async def _refresh_existing_portal_for_user(client: discord.Client, guild_id: int, user_id: int) -> bool:
    ...
    try:
        ...
        if current_title not in main_titles and "gildenzentrale" not in current_title.lower():
            return False
...
def _require_alliance_home_leader(inter: discord.Interaction) -> tuple[bool, str]:
    ...
    if int(inter.guild_id) != int(home_id):
        return False, "❌ Allianz-Raids können nur auf dem Home-Server erstellt werden."
...
async def _dashboard_event_image_bytes(url: str, *, max_bytes: int = 8 * 1024 * 1024) -> bytes | None:
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url, allow_redirects=True, headers={"User-Agent": "GuildBot/2.0"}) as response:
...
async def _dashboard_create_discord_scheduled_event(
    ...
    if not location and channel is not None:
        location = f"#{getattr(channel, 'name', _guild_brand_name(guild) + ' Discord')}"
    location = (location or (_guild_brand_name(guild) + " Discord"))[:100]
    ...
    kwargs: dict[str, Any] = {
        ...
        "reason": f"{_guild_brand_name(guild)} Dashboard/Gildenkalender",
    }
    if image_bytes:
...
async def _dashboard_update_discord_scheduled_event(
    ...
    location = str(payload.get("location") or obj.get("location") or (_guild_brand_name(guild) + " Discord")).strip()[:100]
    kwargs: dict[str, Any] = {
        "name": title,
        "description": description,
        "start_time": start.astimezone(timezone.utc),
        "end_time": end.astimezone(timezone.utc),
        "location": location,
        "reason": f"{_guild_brand_name(guild)} Dashboard/Gildenkalender bearbeitet",
    }
...
async def _dashboard_delete_discord_scheduled_event(guild: discord.Guild, obj: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        scheduled = await guild.fetch_scheduled_event(scheduled_id)
        await scheduled.delete(reason=f"{_guild_brand_name(guild)} Event gelöscht")
...
async def _dashboard_event_create(client: discord.Client, guild_id: int, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not channel_id and central_guild_config is not None:
        try:
            channel_id = int(central_guild_config.channel_id(guild.id, "events") or 0)
        except Exception:
            channel_id = 0
    ...
    if not target_role_id and central_guild_config is not None:
        try:
            member_roles = central_guild_config.role_ids(guild.id, "member")
            target_role_id = member_roles[0] if member_roles else 0
        except Exception:
            target_role_id = 0
...
async def setup_rsvp_dm(client: discord.Client, tree: app_commands.CommandTree):
    ...
    async def _create_alliance_raid_impl(
        ...
        if not home_server:
            await inter.followup.send(
                "❌ Der Home-Server ist in dieser Allianz-Gruppe nicht hinterlegt.\n"
    ...
    @app_commands.describe(
        ...
        time="Zeit HH:MM (24h)",
        description="Kurzbeschreibung (optional)",
        target_role="Optional: Nur diese Home-Rolle bekommt DMs",
        image_url="Optionales Bild fürs Embed"
    )
    ...
    @app_commands.describe(
        ...
        target_role="Optional: andere Home-Gilden-Zielrolle für DMs",
        image_url="Optionales Bild fürs Embed"
    )
    ...
    async def alliance_raid_delete(inter: discord.Interaction, message_id: str):
        ...
        if int(obj.get("guild_id", 0) or 0) != inter.guild_id:
            await inter.followup.send("❌ Dieser Allianz-Raid gehört nicht zu diesem Home-Server.", ephemeral=True)
            return
