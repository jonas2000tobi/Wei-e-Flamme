...
class ScoreView(discord.ui.View, BackToHubMixin):
    ...
    async def my_score(self, inter: discord.Interaction, _btn: discord.ui.Button):
        ...
        # Deinen Score direkt berechnen (sorgt dafür, dass dein Bucket existiert)
        my_total, parts = _calc_flammenscore(gid, uid)

        # Rangliste bauen – dich notfalls hinzufügen, falls du noch nicht im Speicher warst
        data = scores.get(str(gid)) or {}
        scored = [(int(u), _calc_flammenscore(gid, int(u))[0]) for u in data.keys()]
        if not any(u == uid for u, _ in scored):
            scored.append((uid, my_total))
        scored.sort(key=lambda t: t[1], reverse=True)

        pos = next((i for i, (u, _) in enumerate(scored, start=1) if u == uid), 0)

        lines = [
            f"**Rang:** {pos}/{len(scored)}" if pos else f"**Rang:** –/{len(scored)}",
            f"**Score:** {my_total:.1f}",
