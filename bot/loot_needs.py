...
def _normalize_summary_tab(value: str | None) -> str:
    raw = (value or "All").strip().lower()

    if raw in {"all", "alle", "gesamt"}:
        return "All"

    tab = _normalize_tab(raw)
    return tab or "All"


def _need_summary_collect(guild: discord.Guild, tab_filter: str = "All") -> dict[str, list[str]]:
    """
    Sammelt offene Needs nach Itemnamen.
    Erhaltene Items werden ignoriert.
    Gezählt werden nur aktuelle Mitglieder mit gesetzter Gildenmitglied-Rolle.
    """
    tab_filter = _normalize_summary_tab(tab_filter)
    result: dict[str, list[str]] = {}

    members = _current_guild_role_members(guild)

    for member in members:
        data = _user_needs(guild.id, member.id)
        player_name = _profile_name(guild, member.id, member.display_name)

        tabs = TABS if tab_filter == "All" else [tab_filter]
        ...
        for tab in tabs:
            for slot in NEED_SLOTS:
                ...
                if tab_filter == "All":
                    entry = f"{player_name} ({tab})"
                else:
                    entry = player_name

                result.setdefault(item_name, [])

                if entry not in result[item_name]:
                    result[item_name].append(entry)
...
async def _send_need_summary_counts(
    inter: discord.Interaction,
    guild: discord.Guild,
    tab_filter: str = "All",
    public: bool = False,
    ...
    tab_filter = _normalize_summary_tab(tab_filter)
    data = _need_summary_collect(guild, tab_filter)

    title_tab = "Main + Secondary" if tab_filter == "All" else tab_filter

    if not data:
        emb = discord.Embed(
            title="🎁 Need-Zusammenfassung",
            description=f"Keine offenen Needs gefunden.\nBereich: **{title_tab}**",
            color=discord.Color.gold()
        )
        await _send_embed_response(inter, emb, public=public)
        return
    ...
    lines = [
        f"• **{item_name}** — **{len(names)}x**"
        for item_name, names in data.items()
    ]
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        emb.add_field(name="Bereich", value=title_tab, inline=True)
        emb.add_field(name="Items", value=str(len(data)), inline=True)
        emb.set_footer(text=f"Seite {i}/{len(chunks)} • Erhaltene Items werden nicht gezählt.")
...
async def _send_need_summary_names(
    inter: discord.Interaction,
    guild: discord.Guild,
    tab_filter: str = "All",
    public: bool = False,
    ...
    if not data:
        emb = discord.Embed(
            title="🎁 Need-Zusammenfassung mit Spielern",
            description=f"Keine offenen Needs gefunden.\nBereich: **{title_tab}**",
            color=discord.Color.gold()
        )
        await _send_embed_response(inter, emb, public=public)
    ...
    for item_name, names in data.items():
        block = [f"**{item_name}** — **{len(names)}x**"]
        block.extend(f"• {name}" for name in sorted(names, key=lambda x: x.lower()))
        blocks.append("\n".join(block))
...
async def setup_loot_needs(client: discord.Client, tree: app_commands.CommandTree):
    ...
    @tree.command(name="loot_summary_counts", description="(Leader) Zählt offene Needs pro Item")
    @app_commands.choices(tab=_summary_tab_choices())
    async def loot_summary_counts(
        inter: discord.Interaction,
        tab: app_commands.Choice[str] = None,
        public: bool = False,
    ):
        if inter.guild is None:
            await inter.response.send_message("❌ Nur im Server nutzbar.", ephemeral=True)
        ...
        tab_filter = _normalize_summary_tab(tab.value if tab else "All")
        await _send_need_summary_counts(inter, inter.guild, tab_filter, public=public)

    @tree.command(name="loot_summary_names", description="(Leader) Listet offene Needs pro Item mit Spielernamen")
    @app_commands.choices(tab=_summary_tab_choices())
    async def loot_summary_names(
        inter: discord.Interaction,
        tab: app_commands.Choice[str] = None,
        public: bool = False,
    ):
        ...
        tab_filter = _normalize_summary_tab(tab.value if tab else "All")
        await _send_need_summary_names(inter, inter.guild, tab_filter, public=public)
