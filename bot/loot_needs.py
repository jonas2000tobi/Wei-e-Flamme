...
def _guild_name(guild: discord.Guild) -> str:
    try:
...
def _need_embed(guild: discord.Guild, user_id: int, tab: str = "Main") -> discord.Embed:
    ...
    emb = discord.Embed(
        title=f"🎁 Needliste – {_guild_name(guild)}",
        description=(
            f"**{name}**\n"
            f"Bereich: **{tab}**\n\n"
...
async def _send_long_need_list(
    ...
    if not chunks:
        emb = discord.Embed(
            title=f"🎁 Gesamte Needliste – {_guild_name(guild)}",
...
async def setup_loot_needs(client: discord.Client, tree: app_commands.CommandTree):
    ...
    async def loot_reset_all_needs(
        ...
        await inter.response.send_message(
            ...
            f"Scope: **{'Nur aktuelle Gildenrolle' if only_guild_role else 'Alle gespeicherten Spieler'}**",
            ephemeral=True
        )
    ...
    async def loot_need_all(inter: discord.Interaction, public: bool = False):
        ...
        if removed:
            await inter.followup.send(
                f"🧹 Bereinigt: **{removed}** Needliste(n) von Leuten ohne Gildenmitglied-Rolle entfernt.",
                ephemeral=True
            )
    ...
    async def loot_weapon_all(inter: discord.Interaction, public: bool = False):
        ...
        emb = _weapon_summary_embed(
            inter.guild,
            user_ids,
            title="🎯 Waffenbedarf – alle Gildenmitglieder",
            subtitle=f"Quelle: alle Mitglieder mit Gildenmitglied-Rolle\nMitglieder: **{len(user_ids)}**"
        )
