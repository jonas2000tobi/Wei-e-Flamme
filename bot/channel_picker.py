...
def _is_leader_or_admin(inter: discord.Interaction) -> bool:
    ...
    role_id = 0
    central_leader_configured = False
    try:
        if central_guild_config is not None:
            central_leader_configured = central_guild_config.role_mapping_configured(inter.guild.id, "leader")
            ids = central_guild_config.role_ids(inter.guild.id, "leader")
            role_id = ids[0] if ids else 0
    except Exception:
        role_id = 0
        central_leader_configured = False
    if not role_id and not central_leader_configured:
...
def _require_home_leader(inter: discord.Interaction) -> tuple[bool, str]:
    ...
    if not _is_home_guild(inter):
        return False, "❌ Dieser Befehl ist nur auf dem Home-Gildenserver nutzbar."
...
async def setup_alliance_config(client: discord.Client, tree: app_commands.CommandTree):
    ...
    @alliance.command(name="server_add_home", description="(Leader) Home-Gildenserver zu Allianz-Gruppe hinzufügen")
    ...
    async def alliance_partner_register(
        ...
        home_id = _home_guild_id()
        if not home_id:
            await inter.response.send_message("❌ Home-Gildenserver ist noch nicht gesetzt. Auf dem Home-Gildenserver zuerst `/alliance_home_set` ausführen.", ephemeral=True)
            return

        key = _normalize_key(group)
        groups = _groups()
        if key not in groups:
            await inter.response.send_message("❌ Allianz-Gruppe nicht gefunden. Die Gruppe muss zuerst auf dem Home-Gildenserver angelegt werden.", ephemeral=True)
            return
    ...
    @alliance.command(name="event_channel_set", description="(Leader) Eventtyp-Channel für den Home-Gildenserver setzen")
