    ...
    if isinstance(data, dict):
        for message_id, ev in data.items():
            ...
            events.append({
                ...
                "dkp_event_type": _safe_text(ev.get("dkp_event_type") or ev.get("dkp_type") or "", 80),
                "event_type": _safe_text(ev.get("event_type") or "", 80),
                "image_type": _safe_text(ev.get("image_type") or ev.get("image_preset") or "", 40),
                "target_role_id": int(ev.get("target_role_id", 0) or 0),
                "status": _safe_text(ev.get("status") or ev.get("state") or "active", 40),
